...
describe( "Channel REMOVE-BY-ID", function (): void {
  ...
  it( "should remove documents based on id", () => {
    ...
    return storageChannel.removeById( testInstances[ 0 ]._id )
      .then(( response: any ) => {

        ChannelMongooseModel.find().exec(( err, foundDocuments ) => {

          expect( foundDocuments ).to.satisfy(( channels: ChannelModel[] ) => {
            ...
            let culprits = channels.filter(( channel: ChannelModel ) => {
              return ( String( channel._id ) === String( testInstances[ 0 ]._id ) );
            } );
  ...
  it( "should emit event upon removing channel document", () => {

    return storageChannel.removeById( testInstances[ 1 ]._id )
      .then(( response: any ) => {

        sinon.assert.calledOnce( emitEventSpy );

        let emittedEvent: src.events.components.storage.channel.Removed;
        emittedEvent = emitEventSpy.getCall( 0 ).args[ 0 ];

        expect( emittedEvent ).to.satisfy(( happening: src.events.components.storage.channel.Removed ) => {
  ...
  it( "should emit failed event upon erring", () => {
    ...
    return storageChannel.removeById( null, true )
      .then(( response: any ) => {
      ...
      .catch(( reason: any ) => {
        ...
        let emittedEvent: src.events.components.storage.channel.RemoveFailed;
        emittedEvent = emitEventSpy.getCall( 0 ).args[ 0 ];

        expect( emittedEvent ).to.satisfy(( happening: src.events.components.storage.channel.RemoveFailed ) => {
