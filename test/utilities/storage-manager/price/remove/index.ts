...
describe( "Price REMOVE", function (): void {
  ...
  let testInstances: interfaces.dataModel.Price[] = [];
  ...
  it( "should emit event upon removing Price documents", () => {
    ...
    return storagePrice.remove( {
      ...
      .then(( response: any ) => {
        ...
        let emittedEvent: interfaces.events.utilities.storageManager.price.Removed;
        emittedEvent = emitEventSpy.getCall( 0 ).args[ 0 ];

        expect( emittedEvent ).to.satisfy(( happening: interfaces.events.utilities.storageManager.price.Removed ) => {
  ...
  it( "should emit failed event upon erring", () => {
    ...
    return storagePrice.remove( null, true )
      .then(( response: any ) => {
      ...
      .catch(( reason: any ) => {
        ...
        let emittedEvent: interfaces.events.utilities.storageManager.price.RemoveFailed;
        emittedEvent = emitEventSpy.getCall( 0 ).args[ 0 ];

        expect( emittedEvent ).to.satisfy(( happening: interfaces.events.utilities.storageManager.price.RemoveFailed ) => {
