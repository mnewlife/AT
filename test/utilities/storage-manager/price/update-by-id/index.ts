  ...
  it( "should emit event upon updating Price document", () => {
    ...
    return storagePrice.updateById( testInstances[ 1 ]._id, {
      ...
      .then(( updatedDocument: PriceModel ) => {
        ...
        let emittedEvent: src.events.components.storage.price.Updated;
        emittedEvent = emitEventSpy.getCall( 0 ).args[ 0 ];

        expect( emittedEvent ).to.satisfy(( happening: src.events.components.storage.price.Updated ) => {
  ...
  it( "should emit failed event upon erring", () => {
    ...
    return storagePrice.updateById( null, null, true )
      ...
      .catch(( reason: any ) => {
        ...
        let emittedEvent: src.events.components.storage.price.UpdateFailed;
        emittedEvent = emitEventSpy.getCall( 0 ).args[ 0 ];

        expect( emittedEvent ).to.satisfy(( happening: src.events.components.storage.price.UpdateFailed ) => {
