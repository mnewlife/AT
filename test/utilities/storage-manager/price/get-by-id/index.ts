...
it( "should emit event upon retrieving Price document", () => {
  ...
  return storagePrice.getById( testInstance._id )
    .then(( Price: PriceModel ) => {
      ...
      let emittedEvent: src.events.components.storage.price.GotById;
      emittedEvent = emitEventSpy.getCall( 0 ).args[ 0 ];

      expect( emittedEvent ).to.satisfy(( happening: src.events.components.storage.price.GotById ) => {
...
it( "should emit failed event upon erring", () => {
  ...
  return storagePrice.getById( null, true )
    ...
    .catch(( reason: any ) => {
      ...
      let emittedEvent: src.events.components.storage.price.GetByIdFailed;
      emittedEvent = emitEventSpy.getCall( 0 ).args[ 0 ];

      expect( emittedEvent ).to.satisfy(( happening: src.events.components.storage.price.GetByIdFailed ) => {
