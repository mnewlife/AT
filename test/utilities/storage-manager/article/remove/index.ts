...
describe( "Article REMOVE", function (): void {
  ...
  it( "should remove documents meeting specified conditions", () => {
    ...
    return storageArticle.remove( {
      images: [ testInstances[ 0 ].images[ 0 ] ]
    } )
      .then(( response: any ) => {
        ...
        ArticleMongooseModel.find().exec(( err, foundDocuments ) => {
          ...
          expect( foundDocuments ).to.satisfy(( articles: ArticleModel[] ) => {
            ...
            let culprits = articles.filter(( article: ArticleModel ) => {
              return ( String( article.title ) == testInstances[ 0 ].title );
            } );
  ...
  it( "should emit event upon removing article documents", () => {
  ...
  it( "should emit failed event upon erring", () => {
    ...
    return storageArticle.remove( null, true )
      .then(( response: any ) => {
