  ...
  it( "should emit event upon retrieving cart documents", () => {
    ...
    return storageCart.get( null, null, null )
      .then(( foundCarts: interfaces.dataModel.Cart[] ) => {
        ...
        let emittedEvent: interfaces.events.utilities.storageManager.cart.Got;
        emittedEvent = emitEventSpy.getCall( 0 ).args[ 0 ];

        expect( emittedEvent ).to.satisfy(( happening: interfaces.events.utilities.storageManager.cart.Got ) => {
  ...
  it( "should emit failed event upon erring", () => {
    ...
    return storageCart.get( null, null, null, true )
      ...
      .catch(( reason: any ) => {
        ...
        let emittedEvent: interfaces.events.utilities.storageManager.cart.GetFailed;
        emittedEvent = emitEventSpy.getCall( 0 ).args[ 0 ];

        expect( emittedEvent ).to.satisfy(( happening: interfaces.events.utilities.storageManager.cart.GetFailed ) => {
