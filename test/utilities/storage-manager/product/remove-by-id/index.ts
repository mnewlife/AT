...
describe( "Product REMOVE-BY-ID", function (): void {
  ...
  it( "should remove documents based on id", () => {
    ...
    return storageProduct.removeById( testInstances[ 0 ]._id )
      .then(( response: any ) => {
        ...
        ProductMongooseModel.find().exec(( err, foundDocuments ) => {
          ...
          expect( foundDocuments ).to.satisfy(( Products: ProductModel[] ) => {
            ...
            let culprits = Products.filter(( Product: ProductModel ) => {
              return ( String( Product._id ) === String( testInstances[ 0 ]._id ) );
            } );
  ...
  it( "should emit event upon removing Product document", () => {

    return storageProduct.removeById( testInstances[ 1 ]._id )
      .then(( response: any ) => {

        sinon.assert.calledOnce( emitEventSpy );

        let emittedEvent: interfaces.events.utilities.storageManager.product.Removed;
        emittedEvent = emitEventSpy.getCall( 0 ).args[ 0 ];

        expect( emittedEvent ).to.satisfy(( happening: interfaces.events.utilities.storageManager.product.Removed ) => {
  ...
  it( "should emit failed event upon erring", () => {
    ...
    return storageProduct.removeById( null, true )
      .then(( response: any ) => {
      ...
      .catch(( reason: any ) => {
        ...
        let emittedEvent: interfaces.events.utilities.storageManager.product.RemoveFailed;
        emittedEvent = emitEventSpy.getCall( 0 ).args[ 0 ];

        expect( emittedEvent ).to.satisfy(( happening: interfaces.events.utilities.storageManager.product.RemoveFailed ) => {
