...
it( "should emit event upon retrieving Product document", () => {
  ...
  return storageProduct.getById( testInstance._id )
    .then(( Product: ProductModel ) => {
      ...
      let emittedEvent: src.events.components.storage.product.GotById;
      emittedEvent = emitEventSpy.getCall( 0 ).args[ 0 ];

      expect( emittedEvent ).to.satisfy(( happening: src.events.components.storage.product.GotById ) => {
...
it( "should emit failed event upon erring", () => {
  ...
  return storageProduct.getById( null, true )
    ...
    .catch(( reason: any ) => {
      ...
      let emittedEvent: src.events.components.storage.product.GetByIdFailed;
      emittedEvent = emitEventSpy.getCall( 0 ).args[ 0 ];

      expect( emittedEvent ).to.satisfy(( happening: src.events.components.storage.product.GetByIdFailed ) => {
