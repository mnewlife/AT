...
import * as interfaces from "../../../interfaces/groc-round/delivery-fee";
...
export default (
  ...
  let models = new Events<interfaces.Context, interfaces.FiltrationCriteria,
    ...
    dataModel.grocRound.deliveryFee.Super>( emitEvent, "GrocRound|DeliveryFee" );
  ...
  return new ModelController<interfaces.FiltrationCriteria, storage.BaseSortCriteria,
    ...
    dataModel.grocRound.deliveryFee.Super, interfaces.Events>(
...
function makeConditions ( filtrationCriteria: storage.grocRound.deliveryFee.FiltrationCriteria ): Promise<QueryConditions> {
...
function makeSortCriteria ( sortCriteria: storage.grocRound.deliveryFee.SortCriteria ): Promise<string> {
...
function generateUpdateDetails ( document: Model, details: storage.grocRound.deliveryFee.UpdateDetails ): Promise<Model> {
...
function convertToAbstract ( models: Model[], forceThrow = false ): Promise<dataModel.grocRound.deliveryFee.Super[]> {

  return this.checkThrow( forceThrow )
    .then( ( response: any ) => {

      return new Promise<dataModel.grocRound.deliveryFee.Super[]>( ( resolve, reject ) => {

        let returnModels: dataModel.grocRound.deliveryFee.Super[] = [];

        models.forEach( ( model ) => {

          let returnModel: dataModel.grocRound.deliveryFee.Super = {
